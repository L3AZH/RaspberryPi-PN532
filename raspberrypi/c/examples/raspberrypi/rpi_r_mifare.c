/**
  * This example shows connecting to the PN532 and writing a Mifare Classic
  * type RFID tag
  */
#include <stdio.h>
#include <stdint.h>
...
void readBlockNumber(
    ...
    pn532_error = PN532_MifareClassicAuthenticateBlock(&pn532, uid, uid_length,
                                                       blockNumber, MIFARE_CMD_AUTH_A, key_a);
    if (pn532_error) {
        printf("Error when authenticating: 0x%02x\r\n", pn532_error);
        return;
    }
    pn532_error = PN532_MifareClassicReadBlock(&pn532, buff, blockNumber);
    if (pn532_error) {
